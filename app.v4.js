...
function renderHistory() {
    ...
    state.history.slice().reverse().forEach((week, index) => {
        ...
        let rolloverText = '';
        if (week.rollover > 0) {
            rolloverText = `<span class="sub-label" style="color:var(--success); font-size:10px;">+${formatMoney(week.rollover)} rolled over</span>`;
        } else if (week.rollover < 0) {
            rolloverText = `<span class="sub-label" style="color:var(--danger); font-size:10px;">${formatMoney(week.rollover)} debt</span>`;
        }
...
function setupEventListeners() {
    ...
    els.resetWeekBtn.addEventListener('click', () => {
        if (confirm('Start a new week? Remaining budget (positive or negative) will be applied to next week.')) {
